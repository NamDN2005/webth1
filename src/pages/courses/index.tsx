...
// Removed unused Model type declaration

const CoursePage: React.FC = () => {
  const { courses, instructors, saveCourse, saveInstructor, deleteCourse } = useModel('courseModel') as unknown as {
    courses: Course[];
    instructors: Instructor[];
    saveCourse: (course: Partial<Course>) => boolean;
    saveInstructor: (instructor: Partial<Instructor>) => boolean;
  ...
  const columns = [
    ...
    },
    {
      title: 'Trạng thái',
      dataIndex: 'status',
      render: (status: 'active' | 'completed' | 'paused') => {
        const statusMap = {
          active: 'Đang mở',
